...
const ProductCard = ({ id, description, price, shipment, avaliableQuantity, imageDescription, onRemove }) => {
    ...
    return (
        <div className="w-full flex flex-col md:flex-row justify-around items-center border border-gray-200 rounded-md p-4 space-y-4 md:space-y-0">
            ...
            <div className="w-full md:w-3/12 flex flex-col justify-between items-center space-y-2">
                <Counter avaliableQuantity={avaliableQuantity} onCountChange={handleCountChange} />
                ...
                <div className="flex justify-between w-full">
                    ...
                    <span className={shipment ? "font-bold text-gray-900" : "text-green-500"}>{shipment ? "$" + shipment : "Gratis"}</span>
                </div>
            </div>
        </div>
    );
